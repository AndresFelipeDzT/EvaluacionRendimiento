...
* Programa: Multiplicacion de Matrices algoritmo transpuesta (Filas x Filas)
* Version:
    ...
    #pragma omp parallel
    {
        ...
        for (int i = 0; i < D; i++) {
            for (int j = 0; j < D; j++) { //Doble for para recorrer la matriz
                ...
                pB = mB + j * D; //Lo que hay en la columna
...
int main(int argc, char *argv[]) {
    ...
    impMatrix(matrixB, N, 1);  //Muestra la matriz B

    InicioMuestra(); //Inicia el tiempo
    multiMatrixTrans(matrixA, matrixB, matrixC, N); //Multiplica las transpuestas
